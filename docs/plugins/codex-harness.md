...
- Codex app-server `0.147.0` or newer. The plugin still ships and manages the
  ...
  Higher versions initialize with a warning; acceptance permits an attempt and
...
`0.147.0+desktop` does not affect precedence. A higher version is permitted to
initialize rather than treated as proof of compatibility, so startup and
capability operations can still fail with their normal diagnostics.
