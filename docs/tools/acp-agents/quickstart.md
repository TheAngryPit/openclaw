...
`grok-build` is the ACPX agent target that launches the Grok Build CLI, not
the xAI model id `xai/grok-build-0.1`. Pass `grok-build` as `agentId` for ACP;
use the xAI model id only in an OpenClaw model field.
