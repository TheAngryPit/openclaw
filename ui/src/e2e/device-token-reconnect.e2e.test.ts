...
describeControlUiE2e("Control UI device-token reconnect E2E", () => {
  ...
  it("forgets only the current Gateway operator credential and reconnects without credentials", async () => {
    ...
    const wilfredStoreKey =
      `openclaw.device.auth.v1:` + gatewayCredentialScope(WILFRED_GATEWAY_URL);
    const rositaStoreKey = `openclaw.device.auth.v1:` + gatewayCredentialScope(ROSITA_GATEWAY_URL);
    const wilfredSessionTokenKey =
      `openclaw.control.token.v1:` + gatewayOriginScope(WILFRED_GATEWAY_URL);
    const rositaSessionTokenKey =
      `openclaw.control.token.v1:` + gatewayOriginScope(ROSITA_GATEWAY_URL);
