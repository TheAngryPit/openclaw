...
`grok-build` is an ACPX agent target, not the xAI model id
`xai/grok-build-0.1`. Route `grok-build` through `agentId`; use the fully
qualified xAI ref only in an OpenClaw `model` field.
