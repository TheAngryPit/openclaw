...
struct CloudflareAccessTestTokens {
    ...
    static func application(port: Int = 8443) throws -> CloudflareAccessApplication {
        ...
        originURL.host = "gateway.example.test"
        if port != 443 { originURL.port = port }
        return try CloudflareAccessApplication(
            origin: CloudflareAccessOrigin(Self.unwrap(originURL.url)),
    ...
    func session(
        ...
        application: CloudflareAccessApplication? = nil) throws
